...
public class SousCategorieD implements IDao<SousCategorieM> {
	...
	public ArrayList<SousCategorieM> read() {
		...
		try {
			PreparedStatement sql = connect.prepareStatement("SELECT * FROM sousCategorie INNER JOIN categorie ON sousCategorie.idCategorie=categorie.id ORDER BY sousCategorie.titre");
